...
def get_pdb_info(pdb_files):
	...
	for f in pdb_files:
		...
		for chain_id in chains_to_remove:
			str[0].detach_child(chain_id)

		#Check input file is a pairwise interaction
		if len(chains) > 2 or len(chains) == 1:
			error_files.append(f)
			continue
...
if __name__ == "__main__":
	...
	if bool(homodimer_interactions) and not bool(heterodimer_interactions):
		if options.verbose:
			sys.stderr.write("All pdb files contains %i homodimer interactions\n" %(len(homodimer_interactions)))
	...
	#All heterodimer
		...
		if options.verbose:
			sys.stderr.write("All pdb files contains %i heterodimer interactions\n" %(len(heterodimer_interactions)))
	...
	#Both, homodimer and heterodimer
		...
		if options.verbose:
			sys.stderr.write("The pdb file contains %i homodimer and %i heterodimer interactions\n" %(len(homodimer_interactions), len(heterodimer_interactions)))
	
		new_structure = get_structure_homodimer_heterodimer(interactions, homodimer_interactions, heterodimer_interactions, options.verbose)
